...
export class PayMongoService {
  ...
  constructor(private configService: ConfigService) {
    ...
    const publicKey = this.configService.get<string>('PAYMONGO_PUBLIC_KEY');

    // Validate that keys are configured
    if (!secretKey || !publicKey) {
      this.logger.error('❌ PayMongo API keys are not configured!');
      this.logger.error('Please set PAYMONGO_SECRET_KEY and PAYMONGO_PUBLIC_KEY in your environment variables.');
      throw new Error('PayMongo API keys are required. Please configure PAYMONGO_SECRET_KEY and PAYMONGO_PUBLIC_KEY.');
    }

    // Assign after validation (TypeScript now knows they're not undefined)
    this.secretKey = secretKey;
    this.publicKey = publicKey;
    
    // Determine if we're in live mode based on key prefix
    this.isLiveMode = this.secretKey.startsWith('sk_live_');

    // Log configuration status
    if (this.isLiveMode) {
      this.logger.warn('⚠️  PayMongo is configured in LIVE MODE - real payments will be processed!');
    } else {
      this.logger.log('✅ PayMongo is configured in TEST MODE');
    }
    
    this.logger.log(`🔑 Using ${this.isLiveMode ? 'LIVE' : 'TEST'} mode keys`);
  }
